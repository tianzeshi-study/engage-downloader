...
fn get_file_size(host: &str, url: &str) -> u64 {
    ...
    loop {
        ...
        // 检查响应是否为重定向
        if let Some((status_code, headers)) = parse_response_headers(&response) {
            if status_code == 308 {
...
// 解析响应头
fn parse_response_headers(response: &str) -> Option<(u16, std::collections::HashMap<String, String>)> {
    let mut lines = response.lines();
    let status_line = lines.next()?;
    let mut status_parts = status_line.split_whitespace();
    status_parts.next()?; // 跳过HTTP版本
...
fn download_chunk(host: &str, url: &str, start: u64, end: u64, chunk_number: usize, progress: Arc<Mutex<Vec<u64>>>, file_size: u64) {
    ...
    loop {
        ...
        // 检查响应是否为重定向
        if let Some((status_code, headers)) = parse_response_headers(&String::from_utf8_lossy(&response)) {
            if status_code == 308 {
                if let Some(location) = headers.get("Location") {
                    println!("Redirecting to: {}", location);
                    path = location.clone();
...
fn multi_thread_download(url: &str, num_threads: usize) {
    ...
    println!("chunk_size:{}, file_size:{}", chunk_size, file_size);
